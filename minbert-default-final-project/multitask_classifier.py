...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # single score 
        self.paraphrase_linear = nn.Linear(BERT_HIDDEN_SIZE * 2, 1)
        self.paraphrase_dropout = nn.Dropout(config.hidden_dropout_prob)
        self.similarity_linear = nn.Linear(BERT_HIDDEN_SIZE * 2, 1)
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        bert_out = self.bert(input_ids, attention_mask)
        pooler_out = bert_out['pooler_output']
        pooler_out = self.sentiment_dropout(pooler_out)
        cls_logits = self.sentiment_linear(pooler_out)
        return cls_logits
    ...
    def predict_paraphrase(self,
        ...
        pooler_out_1 = bert_out_1['pooler_output']
        bert_out_2 = self.bert(input_ids_2, attention_mask_2)
        pooler_out_2 = bert_out_2['pooler_output']
        pooler_out_1 = self.paraphrase_dropout(pooler_out_1)
        pooler_out_2 = self.paraphrase_dropout(pooler_out_2)
        pooler_out = torch.cat((pooler_out_1, pooler_out_2), dim=-1)
        logit = self.paraphrase_linear(pooler_out) 
        return logit
    ...
    def predict_similarity(self,
        ...
        bert_out_2 = self.bert(input_ids_2, attention_mask_2)
        pooler_out_2 = bert_out_2['pooler_output']
        pooler_out_1 = self.similarity_dropout(pooler_out_1)
        pooler_out_2 = self.similarity_dropout(pooler_out_2)
        pooler_out = torch.cat((pooler_out_1, pooler_out_2), dim=-1)
        logit = self.similarity_linear(pooler_out) 
        return logit
