...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        ...
        # [batch_size, num_heads, seq_len, head_dim] -> [batch-size, num_heads, head_dim, seq_len]
        # [batch_size, num_heads, seq_len, seq_len]
        attention_score = torch.matmul(
            query, key.transpose(2, 3)) / math.sqrt(head_dim)
        attention_score = attention_score + attention_mask
        attention_weight = torch.nn.functional.softmax(attention_score, dim=-1)
        attention_output = torch.matmul(attention_weight, value)
        # [batch_size, num_heads, seq_len, hidden_dim] -> # [batch_size, seq_len, num_heads, head_dim]
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # TODO
        # raise NotImplementedError
        # x -> multi-head self-attention -> add&norm -> feed-forward netword -> add&norm -> out
        x = hidden_states
        attn_output = self.self_attention(x, attention_mask)
        y = self.add_norm(x, attn_output, self.attention_dense,
                          self.attention_dropout, self.attention_layer_norm)
        ffn_output = self.interm_af(self.interm_dense(y))
        out = self.add_norm(y, ffn_output, self.out_dense,
...
class BertModel(BertPreTrainedModel):
    ...
    def embed(self, input_ids):
        ...
        # TODO
        # raise NotImplementedError
        total_embeds = tk_type_embeds + pos_embeds + inputs_embeds
        out = self.embed_dropout(self.embed_layer_norm(total_embeds))
        return out
